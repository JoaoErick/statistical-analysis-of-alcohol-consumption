...
formation = factor(FORMACAO, levels = c(
...
semester = factor(SEMESTRE, levels = c(
...
sex = factor(SEXO, levels = c("Masculino", "Feminino"), labels = c("1", "2")) 

dataset["SEXO"] = matrix(sex, ncol=1)

# FAIXA_ETARIA #

age_group = factor(FAIXA_ETARIA, levels = c(
...
job = factor(TRABALHA, levels = c("Sim", "Não"), labels = c("1", "2"))

dataset["TRABALHA"] = matrix(job, ncol=1)

# ESCOLA_TIPO #

typeSchool = factor(ESCOLA_TIPO, levels = c("Pública", "Privada"), labels = c("1", "2"))

dataset["ESCOLA_TIPO"] = matrix(typeSchool, ncol=1)

# DIRIGE_OU_PILOTA #

driveOrPilot = factor(DIRIGE_OU_PILOTA, levels = c("Sim", "Não"), labels = c("1", "2"))

dataset["DIRIGE_OU_PILOTA"] = matrix(driveOrPilot, ncol=1)

# RENDA_FAMILIAR #

familyIncome = factor(RENDA_FAMILIAR, levels = c(
...
whoDoYouLiveWith = factor(MORA_COM_QUEM, levels = c(
...
religion = factor(RELIGIAO, levels = c(
...
drinkIsImportant = factor(BEBIDA_IMPORTANTE, levels = c("Sim", "Não"), labels = c("1", "2"))

dataset["BEBIDA_IMPORTANTE"] = matrix(drinkIsImportant, ncol=1)

# FAMILIARES_TOMAM_BEBIDAS #

familyMembersDrink = factor(FAMILIARES_TOMAM_BEBIDAS, levels = c("Sim", "Não"), labels = c("1", "2"))

dataset["FAMILIARES_TOMAM_BEBIDAS"] = matrix(familyMembersDrink, ncol=1)

# TOMA_BEBIDA #

consumesDrink = factor(TOMA_BEBIDA, levels = c("Sim", "Não"), labels = c("1", "2"))

dataset["TOMA_BEBIDA"] = matrix(consumesDrink, ncol=1)

# IDADE_TOMA_BEBIDA #

whatAgeDidYouStartDrinking = factor(IDADE_TOMA_BEBIDA, levels = c(
...
howOftenDoYouDrink = factor(FREQUENCIA_TOMA_BEBIDA, levels = c(
...
howManyGlassesDoYouConsume = factor(QUANTOS_COPOS, levels = c(
...
whyDidYouStartConsuming = factor(PORQUE_COMECOU_A_TOMAR, levels = c(
...
whatImageCausesTheDrink = factor(IMAGEM_BEBIDA, levels = c(
...
whatKindOfDrinkDoYouConsume = factor(TIPO_BEBIDA_CONSOME, levels = c(
...
whatReason = factor(MOTIVO, levels = c(
    "Sim", "Outro motivo", "Não consumo"
  ), 
  labels = c(
    "1", "2", "3"
  )
...
humiliationBecauseOfTheDrink = factor(VEXAME_BEBIDA, levels = c(
    "Sim", "Não", "Não consumo"
  ), 
  labels = c(
    "1", "2", "3"
...
drinkingAccident = factor(ACIDENTE_BEBIDA, levels = c(
    "Sim", "Não", "Não consumo"
  ), 
  labels = c(
    "1", "2", "3"
...
droveDrunk = factor(DIRIGIU_BEBADO, levels = c(
    "Sim", "Não", "Não consumo"
  ), 
  labels = c(
    "1", "2", "3"
...
rideWithDrunk = factor(CARONA_COM_BEBADO, levels = c("Sim", "Não"), labels = c("1", "2"))

dataset["CARONA_COM_BEBADO"] = matrix(rideWithDrunk, ncol=1)

# PAIS_BEBEM #

parentsDrink = factor(PAIS_BEBEM, levels = c("Sim", "Não"), labels = c("1", "2"))

dataset["PAIS_BEBEM"] = matrix(parentsDrink, ncol=1)

# ALGUEM_ACIDENTE_BEBIDA #

someoneHadAnAccidentBecauseOfDrinking = factor(ALGUEM_ACIDENTE_BEBIDA, levels = c(
    "Sim", "Não"
  ), 
  labels = c(
    "1", "2"
